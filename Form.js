...
const requestAccessClick = () => { // click function
    alert('Contact Dyson admin to request access to Dyson Prototype Access at Dyson_admin@gmail.com')
}
...
const validateUsername = () => {
    ...
    let isValid = true;
    if (username.value === '') { // Check for empty value
        isValid = false;
...
const validatePassword = () => {
    ...
    if (existingError) { // if exsitingError = true 
    ...
    if (!regex.test(password.value)) {
        ...
        if (!existingError) { // if existingError is false (not exist)
            const passwordError = document.createElement('p');
            passwordError.textContent = 'Password must contain at least 8 character with lower/Upper letter, number, and a special character'
