        ...
        <div class="form-structor">
            <div class="login" :class="pageState.isLoginPage ? '' : 'slide-up'">
                <div class="center">
                    ...
                    <h4 style="text-align: center">xxx SYSTEM</h4>
                    <div class="form-holder">
                        <input type="text" v-model="loginPage.username" class="input" placeholder="username"/>
                        <input type="password" v-model="loginPage.password" class="input" placeholder="password"/>

                    </div>
                    <button class="submit-btn" @click="doLogin">Sign In</button>
                </div>
            </div>
        </div>
    </div>
    ...
    methods: {
        doLogin() {
            this.$verify.emptyCheckObject(this.loginPage).then(obj => {
                ...
                this.$http.login(user).then(res => {
                    if (res.code === 0) {
                        this.$hint.notify.error(res.msg);
                    }else {
                        let token = res.data;
                        this.$http.getUserInfoByToken(token).then(res => {
